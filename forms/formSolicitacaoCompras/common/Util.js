class Util {
  ...
  // Recebe um parâmetro do tipo Date() e retorna se é válido ou não
  static isValid(data) {
    return data.getTime() === data.getTime();
  };
  ...
  static abrirArquivo(idArquivo) {
    window.open(`${top.WCMAPI.serverURL}/portal/p/1/ecmnavigation?app_ecm_navigation_doc=${idArquivo}`, '_blank');
  }
  ...
  static getFluigVersion() {
    return top.WCMAPI.version.substr(0, 3);
  }
  ...
  static destacarAtividadeAtual(...referenciasPainelAtual) {
    referenciasPainelAtual.map((referencia) => {
      ...
      if ($(referencia).children(":first-child")[0].className.includes('panel-default')) {
        setTimeout(() => {
      ...
      } else if ($(referencia)[0].className.includes('panel-default')) {
        setTimeout(() => {
          if (window && window.formController && window.formController._formMode != "VIEW") {
      ...
      $(referencia)[0].className.includes('panel-primary') || $(referencia)[0].className.includes('panel-default') ?
        $(referencia).find(`div[id^='panelCollapse']`).addClass("in") :
  ...
  static converterFloatEmReais(valorEmFloat) {
    return valorEmFloat.toLocaleString("pt-BR", {
      style: "currency",
      currency: "BRL"
    });
  }
  ...
  static criarAutocomplete(seletorElemento, tipo, param) {
    ...
    switch (tipo) {
      case 'autocomplete':
        filterConfig.source.url = '/api/public/ecm/dataset/search?datasetId=dataset_name&searchField=nome&';
        filterConfig.displayKey = 'nome';
        filterConfig.tagMaxWidth = '800px';
        filterConfig.maxTags = 1;
        filterConfig.onMaxTags = (item, tag) => {
          FLUIGC.toast({
            message: 'Somente uma Opção pode ser selecionado.',
            type: 'danger'
          });
        };
        break;
  ...
  static ajustarTitulo(idTable) {
    $('table#' + idTable + ' tbody tr legend').not(':first').each(function (index, element) {
      element.textContent = 'Despesa ' + parseInt(index + 1);
    });
  }
